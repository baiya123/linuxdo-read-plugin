...
- 可设置阅读指定篇数后自动休息一段时间
- 可设置本轮任务最多阅读多少篇帖子，`0` 表示不限制
- 可设置本轮任务最长运行多少分钟，`0` 表示不限制
...
1. 打开 LINUX DO 任意页面。
2. 在右下角面板中设置阅读秒数、间隔秒数、滚动步数、休息规则和本轮停止条件。
3. 点击“开始”，脚本会先前往 `https://linux.do/latest`。
...
| 休息秒数 | 达到阅读篇数后暂停多久再继续 |
| 最大帖子 | 本轮任务最多完整读完多少篇帖子后自动停止；设置为 `0` 表示不限制 |
| 最长分钟 | 本轮任务运行多少分钟后自动停止；设置为 `0` 表示不限制 |
| 主帖赞阈值 | 主帖当前点赞数达到该值时提示并高亮点赞控件；设置为 `0` 表示关闭提示和自动点赞 |
| 自动点赞 | 开启后，仅在脚本运行中且主帖点赞数达到阈值时，自动点击主帖点赞按钮 |
| 收起/展开 | 隐藏面板内容或恢复显示，收起时显示带 LINUX DO 图标的圆形浮标 |
| 开始/停止 | 启用或暂停只读浏览 |
| 清记录 | 清空本地已浏览帖子记录 |
...
- `最大帖子` 和 `最长分钟` 只限制当前这一轮任务；每次点击“开始”都会重新计算本轮已读数量和运行时间。
