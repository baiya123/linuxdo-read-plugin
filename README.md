...
1. 打开 LINUX DO 任意页面。
2. 在右下角面板中设置阅读秒数、间隔秒数、滚动步数，以及读几篇后休息多久。
...
- 只有检测到话题底部后才会计入“今日已读”；未到底的长帖会跳过，不计入已读。
